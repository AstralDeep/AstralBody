...
- [x] T030 [US1] Real-browser end-to-end parity pass — **DONE 2026-06-10** against the live Docker stack (headless Chromium/Playwright): shell+static+auth+WS handshake, text & tool chat round-trips, all 26 primitive types + escaping, FR-012 interactions, streaming merge (SC-007), audio playback, download, live pagination. Evidence + screenshots: [evidence/t030/EVIDENCE.md](evidence/t030/EVIDENCE.md). Found & fixed: py3.11 f-string SyntaxError that broke all live rendering (renderer.py), chat_steps stuck-`in_progress` LogRecord bug, `admin_review` NameError, stale `.env` (`PUBLIC_BASE_URL`, broken `LLM_MODEL`). Chrome-borne surfaces (settings/audit/feedback/tutorials UIs) explicitly deferred to the 027 chrome spec per operator direction.
