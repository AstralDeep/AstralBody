...
### Native hosts
- [ ] T013 Windows `renderer.py`: add `color_picker` + `theme_apply` builders; honor the `ParamPicker` action-submit (submit → `ctx.emit(submit_action, {fields, …})`); expose a `render_components(list) -> QWidget` entry for the modal host.
- [ ] T014 Windows `chrome.py` + `app.py`: implement the surface host (`render_surface_modal` builds a modal from `chrome_surface.components` via `renderer.render_components`); route the four ported surfaces in `_open_surface` (`app.py:924-941`) to emit `chrome_open` + render the returned `chrome_surface` (replace the `QMessageBox` placeholder); consume the `chrome_surface` frame in `_on_message` (`app.py:1090-1145`); retire `chrome_render_notice` (`chrome.py:29-45`).
...
- [ ] T016 Android `:core`: complete `Inbound.ChromeSurface` + `Wire.decode` mapping; add a reducer case in `:app` `AppViewModel.reduce` storing the decoded surface in `UiState` (twin of `Inbound.ChromeMenu` at `AppViewModel.kt:501`).
- [ ] T017 Android `:app`: add a `Screen.Surface` host that renders the held `chrome_surface.components` via the existing `render/Renderer.kt` in a modal/sheet; route `AppViewModel.openMenuItem` (`:317-328`) `else` for the four ported surfaces to it (replace `SurfacePlaceholderScreen`); add `color_picker`/`theme_apply` composables + `ParamPicker` action-submit (`emit.event(submit_action, {fields})`).
- [ ] T018 [P] Android `:core`/`:app` tests: `ChromeSurface` decode (unknown-field tolerant); surface-host maps a component list to the renderer; submit/button → `sendEvent` payload. (`:core` logic Kover ≥90%.)
...
- [ ] T019 [P] [US1] `surfaces/guide.py::components()` — TOC as `Button`s (`chrome_open` w/ `section`) + the selected section's article as `Text`/`Card`; reuse `_visible_sections(roles)` (`guide.py:29-39`). Port the 16 `guide_content.py` articles to SDUI text blocks. Web unchanged.
- [ ] T020 [P] [US1] `surfaces/theme.py::components()` — 5 preset `Card`/`Button`s (`chrome_theme_preset`) with swatch rows + 7 `color_picker`s (fine-tune). Reuse `PRESETS`/`_stored_theme`. Web `render()` unchanged (`components()` native-only).
...
- [ ] T022 [US1] `surfaces/llm.py::components()` — one `ParamPicker` action-submit form (base_url `text`, api_key `password` write-only, model `text`/`select` when `params["models"]`) + action `Button`s (Load models/Test/Save/Clear) + saved/not-configured `Badge`. Reuse `_user_creds`/`_model_field`.
- [ ] T023 [US1] `surfaces/personalization.py::components()` — `Tabs` (soul/memory/skills/schedule/dreaming) → per-tab bodies: soul `ParamPicker` (profession/goals `textarea`/notes `textarea`); memory `List_` of per-row edit (`ParamPicker` + `Button`s); skills catalog rows (`Button` toggles, reason text when scope not granted); schedule job `Card`s (`Badge` status + Pause/Resume/Run/Delete `Button`s + run history); dreaming toggle+trigger `Button`s. Reuse every existing data read.
- [ ] T024 [P] [US1] Backend tests `test_surface_components.py`: each `components()` returns valid `astralprims` dicts using only allowed types; audience filtering (guide admin section); **no `render()` HTML change** (snapshot before/after — all four keep their web HTML); the native menu omits `tour` while the web menu keeps it.
