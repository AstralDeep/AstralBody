...
The four surfaces to port are **LLM settings, Personalization, Theme, and User guide**. (Agents & permissions and Audit log already have native screens; Tool quality and Tutorial admin are admin-only and, per feature 042, remain **web-only**. **Take the tour is removed from the native clients** — see Clarifications — joining the web-only set, because it is a web-DOM-anchored walkthrough with no native analog.)

## Clarifications

### Session 2026-07-01

- Q: How should native settings forms (LLM, Personalization) express savable controls, given `Button` is the only action-carrying primitive? → A: Extend the existing `ParamPicker` with an action-submit mode (+ password/textarea field kinds); per-row toggles/actions use `Button`. No net-new input primitives.
- Q: Is the live native theme restyle (US3, P2) in scope for this feature? → A: Yes — all three user stories ship in this feature (render + save + live native restyle on Windows and Android).
- Q: What is the native-appropriate equivalent for "Take the tour"? → A: None — remove "Take the tour" and its options from the Windows and Android clients entirely (web-only). Native menus show HELP = User guide only; the web keeps the tour unchanged.
- Q: How should a converted surface handle its web render path to guarantee the web is unchanged (SC-006)? → A: Keep every web `render()` HTML exactly as-is; add `components()` for native only (no web convergence).
...
**Independent Test**: On Windows and on Android, open each of the four settings items in turn and confirm a working native surface renders (not a placeholder, not a web view) with the same content the web shows. (Take the tour no longer appears in the native menu.)

**Acceptance Scenarios**:

1. **Given** the Settings menu on a native client, **When** the user opens "Theme" / "User guide" / "LLM settings" / "Personalization", **Then** the corresponding surface renders as native UI with the same fields/sections/controls as the web.
...
Interacting with a ported surface actually does something: choosing a theme preset, saving an LLM setting, editing a personalization/memory item, or toggling a skill posts back to the server and takes effect, exactly as on the web (same server actions, same audit trail).
...
**Independent Test**: On each native client, perform the primary action of each surface (apply a theme preset, save/clear an LLM setting, edit a memory item, toggle a skill) and confirm it takes effect and is reflected/audited as on the web.
...
- **FR-004**: Opening a settings item on a native client MUST show its surface within ~1 second of selection under normal conditions (checked during cross-client verification); the web surface behavior MUST be unchanged by the port.
...
- **FR-009**: **Take the tour** MUST be removed from the Windows and Android clients — the menu item and its options MUST NOT appear on native (the server MUST omit it from the native menu channels, mirroring how feature 042 keeps admin tools web-only). It remains available and unchanged on the web. No native tour surface is delivered.
...
- **Settings surface (SDUI)**: a server-generated, device-adapted screen for one menu item (Theme, User guide, LLM settings, Personalization), composed from the shared primitive vocabulary so any client renders it natively.
- **Surface component**: one node of a surface (text, card, table, list, input, button, color control, tabs, …) — the existing SDUI wire vocabulary the clients already render. The only additive change is an **action-submit mode on the existing `ParamPicker`** (so a settings form posts its collected fields to a server action), plus native renderers for the existing `color_picker`/`theme_apply` primitives — approved and documented before use (FR-016); no new component *type* is introduced.
- **Surface action**: an interaction within a surface (apply preset, save/clear setting, edit/delete memory, toggle skill) that posts back to the server via the existing action channel and hits the same permission/audit gates.
...
- **SC-001**: On both native clients, 100% of the four in-scope settings items (Theme, User guide, LLM settings, Personalization) open a working native surface — **zero** "coming soon" placeholders and **zero** embedded web pages remain for them; **Take the tour** no longer appears in the native menu.
...
- **SC-003**: The primary action of each surface (apply theme preset, save/clear LLM setting, edit a memory item, toggle a skill) succeeds on both native clients and is reflected/audited exactly as on the web.
...
- **Web-only surfaces**: Tool quality and Tutorial admin (admin-only, feature 042) and **Take the tour** (per Clarifications) are not ported; they remain available only on the web. Removing the tour item from the native menu mirrors 042's admin-tools web-only filtering of the native menu channels.
...
- **Incremental, independently-shippable delivery**: surfaces land one at a time (simplest first — User guide/Theme — then LLM settings, Personalization); each is production-ready and verified on every affected client before the next.
