...
- [ ] T001 Scaffold `apple-clients/` (README.md with canonical manual-Xcode setup + optional
      XcodeGen note, `project.yml`, directory layout per plan.md).
- [ ] T002 [P] `apple-clients/AstralCore/Package.swift` — SPM package, platforms iOS 17 /
      macOS 14 / watchOS 10, zero dependencies, test target.
- [ ] T003 [P] `.github/workflows/apple-ci.yml` — macOS runner, `swift test` for AstralCore
      (app-build job added in T052).
- [ ] T004 [P] `.env.example` — `FF_DEVICE_LOGIN`, extended `KEYCLOAK_ALLOWED_AZP` sample
      (`astral-ios,astral-macos,astral-watch`).
- [ ] T005 [P] `docs/keycloak-realm-settings.md` — device-grant enablement on `astral-watch`,
...
- [ ] T006 [P] Tests first: `backend/tests/test_apple_profiles.py` — `ios`/`macos` named
      profiles derive full-capability (no viewport downgrade), `supported_types` negotiation,
      watch profile unchanged.
- [ ] T007 `backend/rote/capabilities.py` — `DeviceType.IOS`/`MACOS` + host-config entries
      (041 `android` pattern). Green T006.
- [ ] T008 [P] Tests first: `backend/tests/test_qr.py` — known-answer matrices (frozen from
...
- [ ] T009 `backend/shared/qr.py` — first-party QR Model 2 byte-mode encoder (ECC M, GF(256)
      Reed–Solomon, standard format-info table) + stdlib PNG writer + `qr_matrix()` accessor.
      Green T008.
- [ ] T010 [P] Tests first: `backend/tests/test_device_login.py` — broker state machine with
...
- [ ] T011 `backend/orchestrator/device_login.py` — RFC 8628 broker per
...
- [ ] T012 Route wiring: `POST /api/auth/device/{start,poll,refresh}` registered alongside the
      existing `/api/auth/logout` (`backend/orchestrator/web_auth.py` / api router), audit
      `auth.device_login_*` actions emitted.
- [ ] T013 [P] Tests first: `backend/tests/test_watch_speech.py` — `speech` attachment helper:
...
- [ ] T014 Speech helper (`backend/orchestrator/watch_speech.py` or colocated in webrender):
...
- [ ] T016 [P] `Sources/AstralCore/Protocol/` — `Frames.swift` (47 push types: typed decode
...
- [ ] T017 [P] `Sources/AstralCore/Transport/WSClient.swift` — URLSession WebSocket,
...
- [ ] T018 [P] `Sources/AstralCore/Auth/PKCE.swift` — verifier/challenge (S256, CryptoKit),
      authorize-URL builder, token exchange/refresh via URLSession; unit tests (RFC 7636
      test vector).
- [ ] T019 [P] `Sources/AstralCore/Auth/DeviceLogin.swift` — start/poll/refresh client with
      state machine mirroring the contract (interval + slow_down honoring); unit tests
      against a stubbed transport.
- [ ] T020 [P] `Sources/AstralCore/Auth/TokenStore.swift` + `Session.swift` — Keychain-backed
      store (protocol-abstracted for tests), silent refresh, `auth_required` handling hooks.
- [ ] T021 [P] `Sources/AstralCore/API/Rest.swift` — chats list/detail, new chat, logout
...
- [ ] T022 [US1] `AstralApp/` app entry + navigation shell (SwiftUI multiplatform; iOS
      presentation) with server-address config + sign-in screen
      (ASWebAuthenticationSession + PKCE, `astral-ios`).
- [ ] T023 [US1] Chat list screen (REST paginated, open/delete, pull-to-refresh).
- [ ] T024 [US1] Chat canvas: transcript rendering, `ui_stream_data` terminal semantics,
...
- [ ] T031 [US2] macOS window anatomy: top bar (identity/status/new-chat/search), chat rail,
      canvas (reuses US1 renderer).
- [ ] T032 [US2] PKCE via ASWebAuthenticationSession (`astral-macos`); session persistence
      across restarts.
...
- [ ] T037 [US3] `AstralWatch/` app entry; signed-out screen: QR (backend PNG; matrix
      re-render optional), short code, expiry countdown.
- [ ] T038 [US3] Poll loop honoring interval/slow_down; auto-rotate before expiry; approved ⇒
      token store + signed-in transition.
- [ ] T039 [P] [US3] Failure states: denied / denied_no_access / expired / 503 unavailable —
      friendly retry UI, no partial sessions.
- [ ] T040 [P] [US3] Signed-in identity display + one-tap sign-out (logout endpoint,
...
- [ ] T042 [US4] Watch home: one-tap new conversation; bounded recents list.
- [ ] T043 [US4] Dictation-first input (TextFieldLink; scribble/keyboard fallback) with
      confirm-before-send.
- [ ] T044 [US4] Watch chat view: crown-scrollable adapted components + narrative.
- [ ] T045 [US4] `Speaker` (AVSpeechSynthesizer): speak `speech.ssml`→`text` fallback on
...
- [ ] T047 [US5] Backend sweep test: all 35 component types (+ nested layouts) through a
...
- [ ] T048 [P] [US5] Watch fallback UX: read-only summary + "continue on another device"
...
- [ ] T054 [P] `docs/production-deployment.md` — Apple client + device-login operational
      notes.
- [ ] T055 [P] `apple-clients/KNOWN-ISSUES.md` (041 convention).
