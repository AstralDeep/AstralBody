...
- [ ] T003 [P] Document the two-flag posture and the T057 inheritance in `specs/056-delegated-agent-chaining/contracts/audit-and-machine-turn.md` cross-check (no code) — confirm the plan's flag table matches `feature_flags.py`
...
- [ ] T026 [P] [US2] Extend the same seam to draft self-tests: `agentic_creation._self_test_draft` (`backend/orchestrator/agentic_creation.py` ≈:323-350) derives machine authority before its `handle_chat_message`; test in `backend/tests/test_machine_turn_classes.py::test_self_test_authority`
- [ ] T027 [US2] Machine-principal attribution in audit (FR-014, SC-005): set the per-turn machine-context marker (turn_class + owning human + consent_ref) so the T006 principal resolution attributes machine-turn tool-call records to `machine:<class>` + human, never "legacy"/"unknown", while cost stays on the SYSTEM LLM credential (054, `_llm_audit_principals` ≈4624-4640); test in `backend/tests/test_machine_principal.py` (all three classes attributed; cost vs authority distinct — US2-AS5)
- [ ] T028 [US2] Machine-root chains use the same child-mint rules (FR-015): a hop initiated inside a machine turn mints children off the consent-derived root exactly as an interactive hop (T015); test in `backend/tests/test_machine_turn_authority.py::test_machine_turn_chains_attenuate`
- [ ] T029 [US2] Ship-dark verification (FR-016): with `FF_SCHEDULER_EXECUTION=0` all US2 machinery is present but inert (no run executes); the review gate is inherited, not bypassed; assert in `backend/tests/test_scheduler_execution_gate.py`
- [ ] T030 [US2] Live verification per quickstart.md §US2 under production posture with `FF_SCHEDULER_EXECUTION=1` in a staging boot: scheduled run dispatches real-agent tools under consent; logout → next run pauses with one notification — evidence under `evidence/us2/`
...
- [ ] T037 [US4] Live verification per quickstart.md §US4 (decomposed request → ≥3 isolated budgeted sub-tasks; MAS quarantine; peer path gone; hierarchical progress on web AND one native client unchanged) — evidence under `evidence/us4/`
