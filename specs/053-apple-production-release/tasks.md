...
- [x] T054 [P] [US7] Revise `../obsidian-vault/wiki/concepts/CI Gates.md` (add `apple-release.yml`), `Feature Flags.md` (reconcile `FF_DEVICE_LOGIN` state), `Feature Timeline.md` (add 053), and fix the stale `../obsidian-vault/wiki/entities/AstralBody.md` hub "Current state" commit (FR-029). **DONE** — `[[CI Gates]]` moves `apple-release.yml` from planned to real; records that `apple-ci.yml` gained exactly one additive step and that `ci.yml` + `release-windows.yml` are byte-identical to main.
- [x] T055 [US7] Bump `../obsidian-vault/wiki/sources/astralbody-repo.md` reviewed_commit to the 053 merge commit; append a `## [2026-07-08] update | 053 apple production release` entry to `../obsidian-vault/log.md`; update `../obsidian-vault/index.md`; run the vault LINT (SC-010). **DONE** — `[[astralbody-repo]]` anchor bumped to `46d6c96`; `log.md` appended; vault committed (`bfdc391`). Honesty preserved: no page claims 053 is shipped, signed or submitted.
