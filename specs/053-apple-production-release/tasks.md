...
- [x] T020 [US1] Validate: archive the **two** targets — iOS (containing the embedded watch app) and macOS — and run `xcrun altool --validate-app` (or App Store Connect API validation) for each; record zero-error output in `specs/053-apple-production-release/verification/us1-validation.md` (SC-001). *(Live account gates the real upload; validation runs against a distribution profile.)* **DONE** — tag `apple-v1.0`, run 29036053155: `VERIFY SUCCEEDED`×2 and `UPLOAD SUCCEEDED`×2; `AstralDeep.ipa` (iOS + embedded watch) and `AstralDeep.pkg` (Mac App Store) both accepted by App Store Connect (Delivery UUIDs b6a0f233…, fd1f2dd6…).
...
- [x] T052 [US7] Create `../obsidian-vault/wiki/entities/Apple Release Pipeline.md` (signing, xcconfig endpoint config, icon generator + `--check`, embedded-companion watch, one-record store topology, App Store submission flow, `apple-release.yml`, secrets) per the vault CLAUDE.md schema (frontmatter, lead, `## Sources` → `[[astraldeep-repo]]`, wikilinks) (FR-029). **DONE** — `[[Apple Release Pipeline]]` rewritten from planned to real: the workflow's steps, the seven secrets by name, the `apple-v*` tag-glob trap, the iOS platform filter, `app-store-connect`, no-notarytool, three profiles, and the icon opacity inversion.
- [x] T053 [P] [US7] Revise `../obsidian-vault/wiki/entities/Apple Clients.md` (retire "legacy keychain until real signing exists"; add signed/MAS/entitlements/privacy-manifest/embedded-companion-watch state) and clear the client-id `> Conflicts with` block on `../obsidian-vault/wiki/entities/Keycloak Realm Astral.md` (FR-029). **DONE** — `[[Apple Clients]]` gap table split into CLOSED vs genuinely-open; records the embedded-companion watch, the opportunistic override, the `https:/$()/host` xcconfig escape, privacy-manifest placement, and why there are no keychain-access-groups or usage strings.
- [x] T054 [P] [US7] Revise `../obsidian-vault/wiki/concepts/CI Gates.md` (add `apple-release.yml`), `Feature Flags.md` (reconcile `FF_DEVICE_LOGIN` state), `Feature Timeline.md` (add 053), and fix the stale `../obsidian-vault/wiki/entities/AstralDeep.md` hub "Current state" commit (FR-029). **DONE** — `[[CI Gates]]` moves `apple-release.yml` from planned to real; records that `apple-ci.yml` gained exactly one additive step and that `ci.yml` + `release-windows.yml` are byte-identical to main.
- [x] T055 [US7] Bump `../obsidian-vault/wiki/sources/astraldeep-repo.md` reviewed_commit to the 053 merge commit; append a `## [2026-07-08] update | 053 apple production release` entry to `../obsidian-vault/log.md`; update `../obsidian-vault/index.md`; run the vault LINT (SC-010). **DONE** — `[[astraldeep-repo]]` anchor bumped to `46d6c96`; `log.md` appended; vault committed (`bfdc391`). Honesty preserved: no page claims 053 is shipped, signed or submitted.
...
- [x] T060 [P] Final Constitution compliance pass (I–XIII), emphasizing X (no hardcoded endpoint, per-client verification incl. the watch no-companion fallback), XI (release workflow additive), XII (drift guard green, parity) — record in the PR description. **DONE** — Constitution pass, evidence-backed: III 74 AstralCore tests (10 new) green; V Package.swift + requirements.txt unchanged vs main, both new scripts stdlib-only, no fastlane/xcodegen; VII no PEM/base64 in any of the 71 changed files, Team ID via `$(ASTRAL_DEVELOPMENT_TEAM)`, secrets referenced by name only; IX `database.py` unchanged; X no TODO/FIXME in new code and 0 localhost strings in the Release binary; XI `ci.yml` + `release-windows.yml` diffs EMPTY, `apple-ci.yml` additive; XII `ui_protocol.json` unchanged and the drift guard green (47/35/67).
