...
- **FR-002**: Each target MUST declare the entitlements its capabilities require (keychain access for token storage; macOS App Sandbox), stored as version-controlled entitlement definitions.
...
- **FR-005**: A privacy manifest MUST declare the app's data-collection and required-reason API usage, and every runtime permission the app uses (e.g. watch speech/dictation) MUST have a user-facing purpose string. Because the embedded watch app is its own `.app` bundle, **each app target ships its own manifest** — `AstralApp/PrivacyInfo.xcprivacy` and `AstralWatch/PrivacyInfo.xcprivacy` (the watch's must declare the `UserDefaults` required-reason category it uses to store the endpoint override).
- **FR-006**: Release network configuration MUST be App-Transport-Security compliant — no blanket arbitrary-loads exception may apply to Release; any localhost/dev exception MUST be scoped to Debug only.
...
- **FR-011b**: Because `AstralApp` is a **single multiplatform target** building both `iphoneos` and `macosx`, the "Embed Watch Content" phase MUST be **platform-filtered to iOS**. An unfiltered phase would attempt to embed a watchOS app into the macOS archive; a macOS app must not contain a watch app.
...
- **FR-015**: A tag-triggered release workflow MUST archive, export, upload, and submit for review the **two** platform builds — iOS (containing the embedded watch app) and macOS — into the single Universal Purchase App Store Connect record, using CI-injected secrets, and MUST fail clearly (without producing an unsigned artifact or leaking secrets) when signing material is missing.
...
- **SC-001a**: Both platform builds are uploaded to the single Universal Purchase App Store Connect record and the version is submitted for App Store review (the submission action completes; Apple's review decision is outside this feature's control) with a complete store listing.
