...
- [ ] T012 Update `backend/verification/drivers/in_process.py` (≈110-117): replace the `OPENAI_*` env fake with seeding harness-namespaced `user_llm_config` rows or `client_factory` injection. `system_llm_config` is a singleton and MUST only be seeded by the in-process driver against an ephemeral DB — external mode against a live deployment must never write it (032 namespaced-principal posture)
...
- [ ] T014 [P] [US1] Add mandatory modal variant to `backend/webrender/chrome/__init__.py::render_modal_shell` — `mandatory=True` omits the ✕ button, stamps `data-mandatory="1"` on `.astral-modal-card`, and includes a "Sign out" link (→ `/auth/logout`) in the modal footer (spec FR-013 escape hatch)
...
- [ ] T022 [US1] Fix stale tour copy in `backend/webrender/chrome/guide_content.py` (≈454) claiming the tour auto-launches on first sign-in (it is user-initiated; first-run ordering now = gate → welcome → tour-on-demand)
