...
**Alternative under consideration — Cresco edge mesh as the persistent transport** (raised 2026-07-14): CrescoEdge's `agent` (Java/OSGi) is purpose-built for persistent, secure, NAT-traversing, multi-tenant connections between distributed edge agents and central controllers — exactly the hardest problem here. It could carry 057 agent frames: `user Python agent → local Cresco agent → Cresco mesh → controller → wsapi bridge → orchestrator`. **Reconciliation with the prior 050 decision** (`specs/050-cresco-integration-decision/`, Approved): 050 said **NO** to platform-level adoption (no JVM in the product **image**, no ActiveMQ internal bus, no protocol replacement — Constitution I + V) and **YES** to a first-party Python **bridge** to an *external* Cresco fabric via `wsapi` (using only the already-present `websockets` lib). That posture is compatible with 057 **only** if: the JVM/Cresco agent runs on the **user's desktop** (not the product image — 050 forbids the image, not the client), the orchestrator reaches the fabric through the existing 050 `wsapi` bridge (server stays JVM-free), and — critically — Cresco provides **transport + machine/tenant identity only**; the 057 per-user owner-binding + RFC 8693 human-anchored delegation + boundary re-verification (US3) still run **unchanged** on top (Cresco has no per-user consent/permission/audit model — 050's core finding). **Trade-off**: Cresco is battle-tested persistence but a **heavy new client-side dependency** (JDK 21 + a Cresco agent process + an operator-run controller), whereas the UI-socket tunnel is **zero-dependency and already persistent + authenticated** (the client holds that socket for the UI regardless), so it solves the NAT/persistence problem by *reuse* rather than by adding a mesh. **Recommendation**: default to the UI-socket tunnel for v1 (zero-dep, Constitution V, offline-on-close for free); keep Cresco as a **pluggable transport** evaluated by a spike (see tasks) and adopted only if a broader edge-compute/agent-mesh story justifies the dependency — landing under the 050 external-infrastructure posture (`FF`-gated, fabric external, bridge-only on the server). This is a genuine architecture fork surfaced to the owner for decision.
...
- **Windows (feasibility HIGH, zero new deps)**: the client is a frozen CPython/PySide6 process. Run the generated agent as a **separate, client-supervised child process** (re-invoke `sys.executable` — or the frozen exe — with a worker-entry flag; the PyInstaller onefile already carries the interpreter), **not** an in-process daemon thread. Process isolation is the resolution to finding C1: an in-process thread would run the generated code with the client's full authority (its memory, session tokens, files), giving zero local isolation; a supervised child process with a restricted working set bounds it to the user's own granted resources and cannot reach the client's own secrets. `aiohttp/websockets` are already pinned in `windows-client/requirements.txt`, so this stays zero-dependency. (The child re-connects its frames through the client's tunnel; the client supervises lifecycle + offline-on-close.)
