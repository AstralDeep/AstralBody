...
**Transport is a seam, not a hard-coding.** The registration/dispatch/offline contract below is defined in terms of *"a frame arrives from an owner-bound connection"* — it is agnostic to how that connection is carried. The **v1 default** is the UI-socket tunnel (zero new dependency; the client already holds a persistent, authenticated socket). A **Cresco edge-mesh transport** (CrescoEdge `agent` on the desktop → mesh → controller → the feature-050 `wsapi` bridge → orchestrator) is a considered alternative that satisfies this same contract *if adopted*: it changes only how frames arrive, not the owner-binding, boundary re-verification, or offline rules — because Cresco supplies transport + machine/tenant identity only, never the per-user grant model. Choosing Cresco is gated on the transport spike (see tasks) and lands under the 050 external-infrastructure posture (JVM never in the product image; server reaches the fabric via the Python `wsapi` bridge only). Owner-binding still derives the human principal from AstralDeep's own record, not from any Cresco/mesh identity.
