...
- **ROTE**: add `android` profile (D8). **Auth**: `astral-mobile` public client **created + allow-listed (operator — done)**; remaining = add an Android **custom-scheme redirect** (`com.astralbody.mobile:/oauth2redirect`) to that client, and document it in `.env.example` + `docs/keycloak-android-client-setup.md`. **CI**: add `android-ci.yml`. **No** new wire protocol, **no** schema change, **no** new backend runtime dependency, **no** `astralprims` change, **no** agent-code change.
