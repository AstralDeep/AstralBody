...
**Testing**: pytest (the two invocations from CLAUDE.md — repo default suite + module suites), run inside the `astraldeep` container against postgres. New tests under `backend/tests/attachments/`, `backend/tests/chrome/`, `backend/agents/general/tests/file_tools/`, and orchestrator unit tests.
