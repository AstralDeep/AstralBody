...
**Testing**: pytest inside the `astralbody` container against live Postgres (default suite + module suites); new suites for ui_designer, workspace layout persistence, ml_services, web_research, summarizer, removal regression; CI runs the same suites in the built image against a postgres:17-alpine service.
