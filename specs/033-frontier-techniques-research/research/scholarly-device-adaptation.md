...
> in **AstralDeep's ROTE** middleware.
>
> AstralDeep today: server-driven UI (astralprims defines → orchestrator renders →
...
- **Cross-surface *distribution* (one session, many devices) is unexplored.** Vulture (INFOCOM'24, fine-grained GUI distribution) + CAMELEON/plasticity foundations show a workspace can span watch+phone+TV with components migrating. AstralDeep's per-component `workspace_layout` overlay model is unusually well-positioned to distribute *components* across a user's simultaneous device sockets (F12, F13).
...
- **AstralDeep gap**: astralprims components *are* server-owned (good), but there's no notion of a per-component **fallback contract** (what to render if a target can't handle me) co-located with the component, and component identity is fingerprint/`wc_*`-based rather than semantic. The "name after product not layout" lesson is relevant to how garnish/`dg_*` ids and component identities are assigned.
- **Priority**: P2.
- **How to implement**: Let each astralprims type (or its renderer entry) declare a **`fallback_type`** chain consumed by ROTE (F1's ladder, but authored once at the component/renderer level rather than hard-coded in ROTE). Keep AstralDeep's existing identity model; add only the fallback metadata as a renderer-registry field. No primitive class change required (renderer-side).
...
- **AstralDeep gap**: ROTE is exactly the rules/scripts approach AUIT argues against — a per-device-type transform branch. There is no objective model (e.g. "fit-to-width", "minimize taps", "glanceability", "speakability") nor a solver selecting among arrangement options; the adaptive **UI-designer** (`ui_designer.py`) optimizes *arrangement* but is device-blind (it never sees the target's constraints).
...
- **AstralDeep gap**: ROTE never reasons about *whether* a given rendering is appropriate for the context — it transforms structurally and stops. AstralDeep already has the `_call_llm` seam and an LLM design loop, but neither scores device/context-appropriateness of an arrangement.
- **Priority**: P1.
...
- **AstralDeep gap**: The **VOICE target is "spoken-friendly text extraction"** only — flat narration, no navigable structure, no audio cues, no per-component verbosity tiers. astralprims `table`/`chart`/`timeline`/`keyvalue` collapse to prose.
- **Priority**: P1 (the single most under-built target; pure-novelty for an agentic SDUI platform).
- **How to implement**: Build a real **VOICE renderer** (`webrender.register_target("VOICE", …)`) that maps each astralprims type to a *structured auditory tree*: SSML with headings/landmarks, a navigable outline (axis→series→point for charts, header→rows for tables), **three verbosity tiers** (TL;DR / key points / full — AstralDeep already produces TL;DR/Key-points/Quotes tabs in the summarizer agent, reuse that LoD habit), and deterministic **earcon markers** (text tokens the device maps to non-speech cues, e.g. `[earcon:up]` for an upward trend computed server-side). All server-side string/markup generation in Python — no audio library, no new dep. New renderer = SDUI-sanctioned.
...
- **AstralDeep gap**: No notion of conveying *trend/shape* of data on any non-visual or small target; numeric series in `chart`/`timeline` are lost on VOICE/WATCH.
...
- **AstralDeep gap**: AstralDeep resolves *one* model via `llm_config.client_factory`; ROTE knows the device but **never uses device class to pick a model tier**. A watch/mobile turn pays the same heavy-model latency as a desktop turn; no escalation policy; no privacy-driven keep-on-device choice.
- **Priority**: P1.
...
- **AstralDeep gap**: ROTE adapts only on **device type**, captured **once**. No use of connectivity, ambient/social context, or user interruptibility — and AstralDeep has scheduled-job + notification machinery (feature 030) that pushes to chat with *no* situational gating.
- **Priority**: P2.
...
- **AstralDeep gap**: ROTE adaptation is **one-shot at `register_ui`**; a browser resize, rotation, or light/dark toggle never re-triggers server adaptation (the per-chat `workspace_layout`/canvas re-render exists but isn't viewport-driven).
- **Priority**: P2.
...
- **AstralDeep gap**: AstralDeep fans `ui_upsert` to *all of a user's sockets on a chat* with per-socket ROTE — but every socket gets the *same* canvas. There's no model for **distributing different components to different devices** (controls on phone, big view on TV) in one session.
...
- **AstralDeep gap**: TV is "passthrough-ish" with bigger fonts; no companion-input model — a TV user can't drive a chat turn from their phone within the same AstralDeep session.
- **Priority**: P3.
...
- **AstralDeep gap**: ROTE renders astralprims to **HTML** and runs an **LLM UI-design loop** — both are exactly the surfaces W4A flags, yet there's no WCAG enforcement at render time or in the designer's validators (which currently only check the *primitive palette*, not accessibility).
...
- **AstralDeep gap**: astralprims already carry semantics (a `Table` *is* a table) — richer than HTML — but ROTE discards that to flat HTML/voice text instead of emitting a **machine-navigable semantic tree** that a VOICE/AT target could traverse.
...
- **AstralDeep gap**: ROTE trims per device type with bespoke logic; there's no **explicit LoD ladder** per component, so WATCH/VOICE degradation is lossy and inconsistent. (FR-027's short-vs-long narrative split is a 2-level hint of this but isn't generalized.)
...
- **AstralDeep gap**: AstralDeep narrates *and* renders the same content regardless of surface; on VOICE the visual is dropped, on WATCH the prose is dropped — but there's no policy that **chooses the modality** (e.g., on VOICE, prefer a 1-sentence answer + offer "want details?"; on TV, prefer visual-first).
- **Priority**: P3.
...
- **AstralDeep gap**: The TV target lacks a **focus/navigation model** — server emits no spatial order, so D-pad navigation isn't possible; "TV" is effectively a larger browser render.
- **Priority**: P3.
...
- **AstralDeep gap**: ROTE's per-target behavior is **code**, not **data**; there's no per-target "host config" (spacing/contrast/max-density/supported-types) that an operator can tune without editing `rote/adapter.py`.
