...
def get_agent_template(agent_name: str, session: Dict[str, Any]) -> str:
    ...
    with open(os.path.join(TEMPLATE_DIR, 'general_agent.py'), 'r', encoding='utf-8') as f:
        template = f.read()
    ...
    template = template.replace("from agents.general.mcp_server import MCPServer", f"from {agent_name}_server import {class_server_name}")
...
def get_server_template(agent_name: str) -> str:
    ...
    with open(os.path.join(TEMPLATE_DIR, 'mcp_server.py'), 'r', encoding='utf-8') as f:
        template = f.read()
    ...
    template = template.replace("from agents.general.mcp_tools import TOOL_REGISTRY", f"from {agent_name}_tools import TOOL_REGISTRY")
...
def get_tools_template() -> str:
    ...
    try:
        with open(os.path.join(TEMPLATE_DIR, 'mcp_tools.py'), 'r', encoding='utf-8') as f:
            return f.read()
    except Exception:
        return '''"""
