...
logger = logging.getLogger('GrantsAgent')
...
def find_available_port(start_port: int = BASE_PORT, max_offset: int = MAX_PORT_OFFSET) -> int:
    ...
    return start_port + 4  # Default offset for grants agent
...
class GrantsAgent:
    ...
    def __init__(self, port: int = None):
        self.agent_id = "grants-1"
        self.service_name = "Grant Searching Agent"
        ...
        if port is not None:
            ...
            env_port = os.getenv("GRANTS_AGENT_PORT")
            if env_port:
    ...
    def _build_agent_card(self) -> AgentCard:
        ...
        for name, info in self.mcp_server.tools.items():
            ...
            skills.append(AgentSkill(
                ...
                tags=["grants", "funding", "nsf", "nih", "doe", "dod", "research"]
            ))
