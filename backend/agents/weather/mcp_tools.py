#!/usr/bin/env python3
"""
MCP Tools for Weather Agent — tool functions that return A2UI Nodes.
...
from shared.a2ui_builders import (
    ...
    plotly_chart, create_response, Node,
)
...
def geocode_location(
    ...
    **kwargs
    ...
    Returns:
        Dict with _a2ui_components, _a2ui_root_id, and _data keys.
    """
    ...
    if cache_key in _geocoding_cache:
        ...
        ui = card("Location Coordinates", [
            alert(
                f"Found coordinates for {city}, {state or country} (cached)",
                variant="info"
            ),
            row([
                metric_card(
                    "Latitude",
                    f"{cached_result['latitude']:.4f}\u00b0",
                    id="lat-metric"
                ),
                metric_card(
                    "Longitude",
                    f"{cached_result['longitude']:.4f}\u00b0",
                    id="lon-metric"
                ),
            ]),
            text(
                f"Location: {cached_result['name']}, {cached_result.get('admin1', '')}, {cached_result.get('country', '')}",
                variant="caption"
            )
        ], id="geocode-card")
        return create_response(ui, data=cached_result)
    ...
    try:
        ...
        ui = card("Location Coordinates", [
            ...
            row([
                metric_card(
                    "Latitude",
                    f"{location_data['latitude']:.4f}\u00b0",
                    id="lat-metric"
                ),
                metric_card(
                    "Longitude",
                    f"{location_data['longitude']:.4f}\u00b0",
                    id="lon-metric"
                ),
            ]),
            text(
                f"Location: {location_data['name']}, {location_data.get('admin1', '')}, {location_data.get('country', '')}",
                variant="caption"
            ),
            text(
                f"Timezone: {location_data.get('timezone', 'Unknown')}",
                variant="caption"
            )
        ], id="geocode-card")

        return create_response(ui, data=location_data)
...
def get_current_weather(
    ...
    **kwargs
    ...
    try:
        ...
        ui = card(f"Current Weather - {location_str}", [
            row([
                metric_card(
                    "Temperature",
                    f"{current.get('temperature_2m', 'N/A')}\u00b0F",
                ...
                metric_card(
                    "Wind",
                    f"{current.get('wind_speed_10m', 'N/A')} mph",
                    subtitle=f"Direction: {current.get('wind_direction_10m', 'N/A')}\u00b0",
                    id="wind-metric"
                ),
                metric_card(
                    "Pressure",
                    f"{current.get('pressure_msl', 'N/A')} hPa",
                    id="pressure-metric"
                ),
            ]),
            alert(
            ...
            text(
                f"Last updated: {current.get('time', 'Unknown')}",
                variant="caption"
            )
        ...
        return create_response(
            alert(
                f"Failed to get current weather: {str(e)}",
                variant="error"
            )
        )
    ...
    **kwargs
    ...
    try:
        ...
        main_metrics = row([
            ...
            metric_card(
                "Sunrise",
                extended.sunrise.split('T')[1][:5] if extended.sunrise else "N/A",
                subtitle="Local time",
                id="sunrise-metric"
            ),
            metric_card(
                "Sunset",
                extended.sunset.split('T')[1][:5] if extended.sunset else "N/A",
                subtitle="Local time",
                id="sunset-metric"
            ),
        ])
        ...
        if extended.pm10 is not None:
            extra_metrics.append(metric_card("PM10", f"{extended.pm10} \u03bcg/m\u00b3", id="pm10-metric"))
        if extended.carbon_monoxide is not None:
            extra_metrics.append(metric_card("CO", f"{extended.carbon_monoxide} \u03bcg/m\u00b3", id="co-metric"))
        if extended.nitrogen_dioxide is not None:
            extra_metrics.append(metric_card("NO\u2082", f"{extended.nitrogen_dioxide} \u03bcg/m\u00b3", id="no2-metric"))

        card_children = [main_metrics]
        if extra_metrics:
            card_children.append(row(extra_metrics))
        card_children.append(
            alert(
                "Data provided by Open-Meteo. UV index and air quality may not be available for all locations.",
                variant="info"
            )
        )

        ui = card(f"Extended Weather - {location_str}", card_children, id="extended-weather-card")
        ...
        return create_response(
            alert(
                f"Failed to get extended weather: {str(e)}",
                variant="error"
            )
        )
    ...
    **kwargs
    ...
    try:
        ...
        if not times:
            return create_response(
                alert(
                    "No historical weather data available for the specified date range.",
                    variant="warning"
                )
            )
        ...
        ui = card(f"Historical Weather - {location_str}", [
            plotly_chart(
                "Temperature Trend",
                chart_data,
                layout={
            ...
            alert(
                f"Data from {start_date} to {end_date or start_date}. Provided by Open-Meteo Historical API.",
                variant="info"
            )
        ...
        return create_response(
            alert(
                f"Failed to get historical weather: {str(e)}",
                variant="error"
            )
        )
    ...
    **kwargs
    ...
    try:
        ...
        if "_data" not in geocode_result:
            return create_response(
                alert(
                    "Could not geocode location to determine state.",
                    variant="error"
                )
            )
        ...
        if country_code != "US":
            return create_response(
                alert(
                    f"Weather alerts are currently only available for US locations. Country: {country_code}",
                    variant="warning"
                )
            )
        ...
        if not state_abbr:
            return create_response(
                alert(
                    "Could not determine state abbreviation for location.",
                    variant="error"
                )
            )
        ...
        if not features:
            return create_response(
                alert(
                    f"No active weather alerts for {admin1} ({state_abbr}).",
                    variant="info"
                )
            )
        ...
        for feature in features[:5]:  # Limit to 5 alerts
            ...
            title_text = props.get("headline", "No title")
        ...
        for wa in alerts_list:
            ...
            variant = variant_map.get(wa.severity, "default")
            alert_components.append(
                alert(
                    f"{wa.title} - {wa.area}",
                    variant=variant,
                    title=f"Severity: {wa.severity.capitalize()}",
                    id=f"alert-{wa.title[:20]}"
                )
            )
            alert_components.append(
                text(
                    wa.description[:200] + ("..." if len(wa.description) > 200 else ""),
                    variant="caption"
                )
            )

        ui = card(f"Weather Alerts - {location_str}", alert_components, id="weather-alerts-card")

        return create_response(ui, data={
            "location": location_str,
            "coordinates": {"latitude": lat, "longitude": lon},
            "state": state_abbr,
            "alerts": [{"title": a.title, "severity": a.severity, "area": a.area} for a in alerts_list],
        ...
        return create_response(
            alert(
                f"Failed to get weather alerts: {str(e)}",
                variant="error"
            )
        )
    ...
    **kwargs
    ...
    try:
        ...
        def fetch_one(loc):
            try:
                ...
                if "_data" in result:
                    ...
                    # Error case
                    return {"error": result.get("_a2ui_components", [])}
            except Exception as e:
                return {"error": str(e)}
        ...
        if errors:
            ...
            return create_response(
                alert(
                    f"Failed to fetch weather for some locations: {error_messages}",
                    variant="error"
                )
            )
        ...
        metric_cards = []
        for i, (name, loc_data) in enumerate(zip(location_names, results)):
            ...
            metric_cards.append(
                card(f"{name}", [
                    ...
                    alert(
                        loc_data.get("weather_description", "No description"),
                        variant="default"
                    )
        ...
        table_rows = []
        for name, loc_data in zip(location_names, results):
            current = loc_data.get("current", {})
            table_rows.append([
                name,
                ...
                loc_data.get("weather_description", "N/A")
            ])

        ui = card("Weather Comparison", [
            plotly_chart(
            ...
            alert(
                f"Comparing {len(location_names)} locations. Data fetched at {datetime.now().strftime('%H:%M:%S')}.",
                variant="info"
            )
        ], id="comparison-card")
        ...
        return create_response(
            alert(
                f"Failed to compare locations: {str(e)}",
                variant="error"
            )
        )
...
def get_hourly_forecast(
    ...
    **kwargs
    ...
    try:
        ...
        ui = card(f"{hours}-Hour Forecast - {location_str}", [
            plotly_chart(
                "Temperature Trend",
                chart_data,
                layout={
        ...
        return create_response(
            alert(
                f"Failed to get hourly forecast: {str(e)}",
                variant="error"
            )
        )
...
def get_daily_forecast(
    ...
    **kwargs
    ...
    try:
        ...
        ui = card(f"{days}-Day Forecast - {location_str}", [
            plotly_chart(
        ...
        return create_response(
            alert(
                f"Failed to get daily forecast: {str(e)}",
                variant="error"
            )
        )
...
def get_weekly_forecast(
    ...
    **kwargs
    ...
    try:
        ...
        ui = card(f"Weekly Forecast Summary - {location_str}", [
            row([
                metric_card(
                    "Average High",
                    f"{avg_high:.1f}\u00b0F",
            ...
            text(
                "This week's forecast shows daily temperature ranges and precipitation totals.",
                variant="body"
            )
        ...
        return create_response(
            alert(
                f"Failed to get weekly forecast: {str(e)}",
                variant="error"
            )
        )
