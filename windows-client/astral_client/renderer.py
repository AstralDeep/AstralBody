...
_TEXT_SIZES = {"h1": (26, True), "h2": (21, True), "h3": (17, True),
               "body": (14, False), "caption": (12, False)}
...
def _r_card(c, ctx):
    ...
    if title:
        ...
        bar = QLabel(); bar.setFixedSize(4, 18)
...
def _r_container(c, ctx):
    w = QWidget(); lay = _vbox(10); w.setLayout(lay)
    _render_into(lay, _children(c), ctx)
    return w


def _r_grid(c, ctx):
    w = QWidget(); grid = QGridLayout(); grid.setSpacing(10)
    grid.setContentsMargins(0, 0, 0, 0); w.setLayout(grid)
...
def _r_badge(c, ctx):
    color, bg = T.VARIANT_COLORS.get(c.get("variant", "default"), T.VARIANT_COLORS["default"])
    lab = QLabel(str(c.get("label", "")))
    lab.setStyleSheet(f"color:{color}; background:{bg}; border:1px solid {color};"
...
def _r_metric(c, ctx):
    ...
    if c.get("delta"):
        row.addWidget(_label(str(c["delta"]), color=T.VARIANT_COLORS["success"][0], size=13, bold=True))
    row.addStretch(1); lay.addLayout(row)
    if c.get("subtitle"):
...
def _r_keyvalue(c, ctx):
    frame = _card_frame()
    lay = _vbox(8, (16, 14, 16, 14)); frame.setLayout(lay)
    if c.get("title"):
    ...
    for i, item in enumerate(c.get("items", []) or []):
        if not isinstance(item, dict):
            continue
        k = _label(str(item.get("label", "")).upper(), color=T.MUTED, size=11, bold=True)
...
def _r_timeline(c, ctx):
    ...
    for item in c.get("items", []) or []:
        ...
        dot = QLabel("●"); dot.setStyleSheet(f"color:{T.PRIMARY}; font-size:10px;")
        row.addWidget(dot, 0, Qt.AlignmentFlag.AlignTop)
        col = _vbox(0)
        if item.get("time"):
            col.addWidget(_label(item["time"], color=T.MUTED, size=11))
        col.addWidget(_label(item.get("title", item.get("label", "")), size=13, bold=True))
        if item.get("description"):
            col.addWidget(_label(item["description"], color=T.MUTED, size=12))
        row.addLayout(col); row.addStretch(1); lay.addLayout(row)
    return frame
...
def _r_rating(c, ctx):
    ...
    frame = _card_frame()
    lay = _vbox(4, (16, 12, 16, 12)); frame.setLayout(lay)
    if c.get("label") or c.get("title"):
        lay.addWidget(_label(c.get("label") or c.get("title"), size=13, bold=True))
    row = QHBoxLayout(); row.setSpacing(8)
    sl = QLabel(stars); sl.setStyleSheet(f"color:{T.VARIANT_COLORS['warning'][0]}; font-size:18px;")
    row.addWidget(sl)
...
def _r_code(c, ctx):
    ...
    edit.setStyleSheet(f"background:{T.SURFACE_2}; border:1px solid {T.BORDER}; border-radius:8px;"
...
def _r_progress(c, ctx):
    w = QWidget(); lay = _vbox(4); w.setLayout(lay)
    if c.get("label"):
    ...
    bar.setStyleSheet(f"QProgressBar{{background:{T.SURFACE_2}; border:1px solid {T.BORDER};"
...
def _r_list(c, ctx):
    frame = _card_frame()
    lay = _vbox(6, (16, 12, 16, 12)); frame.setLayout(lay)
    if c.get("title"):
        lay.addWidget(_label(c["title"], size=14, bold=True))
    ...
    for i, item in enumerate(c.get("items", []) or []):
        ...
        row = QHBoxLayout(); row.setSpacing(8)
        b = _label(bullet, color=T.MUTED); b.setFixedWidth(20)
...
def _r_table(c, ctx):
    ...
    rows = c.get("rows", []) or []
    frame = _card_frame()
    lay = _vbox(8, (12, 12, 12, 12)); frame.setLayout(lay)
...
def _r_tabs(c, ctx):
    ...
    for tab in c.get("tabs", []) or []:
        if not isinstance(tab, dict):
            continue
        page = QWidget(); pl = _vbox(10, (12, 12, 12, 12)); page.setLayout(pl)
...
def _r_collapsible(c, ctx):
    frame = _card_frame()
    lay = _vbox(6, (12, 10, 12, 10)); frame.setLayout(lay)
    body = QWidget(); bl = _vbox(8); body.setLayout(bl)
    _render_into(bl, _children(c), ctx)
    body.setVisible(bool(c.get("default_open")))
    btn = QPushButton(("▾ " if body.isVisible() else "▸ ") + str(c.get("title", "Details")))
    btn.setStyleSheet("text-align:left; border:none; background:transparent; font-weight:600;")

    def toggle():
        body.setVisible(not body.isVisible())
        btn.setText(("▾ " if body.isVisible() else "▸ ") + str(c.get("title", "Details")))
...
def _r_chart(c, ctx):
    ...
    try:
        ...
        pass
    frame = _card_frame(bg=T.SURFACE_2)
    lay = _vbox(4, (16, 14, 16, 14)); frame.setLayout(lay)
...
def _r_chat_history(c, ctx):
    frame = _card_frame()
    lay = _vbox(4, (10, 10, 10, 10)); frame.setLayout(lay)
    if c.get("title"):
        lay.addWidget(_label(c["title"], size=14, bold=True))
    for item in c.get("items", c.get("chats", [])) or []:
        ...
        if cid:
            btn.clicked.connect(lambda _=False, x=cid: ctx.emit("load_chat", {"chat_id": x}))
        lay.addWidget(btn)
    return frame


def _r_skeleton(c, ctx):
    w = QWidget(); lay = _vbox(8); w.setLayout(lay)
    for _ in range(min(6, int(c.get("count", 3) or 3))):
        bar = QLabel(); bar.setFixedHeight(14)
...
def _r_fallback(c, ctx):
    frame = QFrame()
    frame.setStyleSheet(f"background:{T.SURFACE_2}; border:1px dashed {T.BORDER}; border-radius:8px;")
    lay = _vbox(2, (12, 8, 12, 8)); frame.setLayout(lay)
    lay.addWidget(_label(f"[{c.get('type', '?')}]", color=T.MUTED, size=12, bold=True))
    for k in ("title", "label", "message", "content", "value"):
        if isinstance(c.get(k), str):
            lay.addWidget(_label(c[k], color=T.TEXT, size=13)); break
    return frame


REGISTRY: Dict[str, Callable[[dict, RenderContext], QWidget]] = {
    "text": _r_text, "card": _r_card, "container": _r_container, "grid": _r_grid,
    "hero": _r_hero, "badge": _r_badge, "metric": _r_metric, "keyvalue": _r_keyvalue,
    "timeline": _r_timeline, "rating": _r_rating, "alert": _r_alert, "button": _r_button,
    "code": _r_code, "divider": _r_divider, "progress": _r_progress, "list": _r_list,
    "table": _r_table, "tabs": _r_tabs, "collapsible": _r_collapsible,
    "bar_chart": _r_chart, "line_chart": _r_chart, "pie_chart": _r_chart,
    "chat_history": _r_chat_history, "skeleton": _r_skeleton,
}
...
def render(component: Any, ctx: RenderContext) -> QWidget:
    ...
    try:
        ...
        widget = _label(f"[render error: {component.get('type')}: {exc}]", color=T.MUTED, size=12)
